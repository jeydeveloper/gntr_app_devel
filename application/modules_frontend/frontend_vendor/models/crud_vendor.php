...
class Crud_vendor extends CI_Model {
	//--------base---------------
	...
	function get_row(){
		return $this->db->get('vendor')->row_array();
	}
	...
	function delete($data){
		return $this->db->delete('vendor', $data);
	}

	function get_option() {
		$res = $this->where('vndr_void = 0')->get_all();
		$data = array();
		foreach ($res as $key => $value) {
			$data[] = array(
				'name' 	=> $value['vndr_nama'],
				'value' => $value['vndr_id'],
			); 
		}
		return $data;
	...
	function get_option_info() {
		...
		$data = array();
		foreach ($res as $key => $value) {
			$data[$value['vndr_id']] = $value['vndr_nama']; 
		}
		return $data;
	}
}  